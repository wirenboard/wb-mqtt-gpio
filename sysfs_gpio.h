...
class TSysfsGpio
{
  public:
    ...
    inline int SetOutput(bool state = false)
    {
    ...
    inline int GetCachedValue()
    {
        return CachedValue;
    };
    inline void SetCachedValue(int cached)
    {
        CachedValue = cached;
    }; // sometimes we should save written cached value
    inline int GetCachedValueOrDefault(int def = 0)
    {
        return (CachedValue < 0) ? def : CachedValue;
    }

...
{
    TGpioDriverConfig cfg = LoadConfig(testRootDir + "/good1/wb-mqtt-gpio.conf", testRootDir + "/good1/optional.conf", schemaFile);
...
{
    TGpioDriverConfig cfg = LoadConfig(testRootDir + "/good2/wb-mqtt-gpio.conf", "", schemaFile);
    ASSERT_EQ(cfg.DeviceName, "Discrete I/O");
...
{
    TGpioDriverConfig cfg = LoadConfig(testRootDir + "/good3/wb-mqtt-gpio.conf", "", schemaFile);
    ASSERT_EQ(cfg.DeviceName, "Discrete I/O");
    ASSERT_EQ(cfg.Chips.size(), 1);
    ASSERT_EQ(cfg.Chips[0].Lines.size(), 1);
    ASSERT_EQ(cfg.Chips[0].Path, "/dev/gpiochip2");
